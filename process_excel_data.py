...
def process_data(input_file, output_file=None):
    ...
    if 'sys_created_on' not in df.columns or 'u_service_offering_outage_start_time' not in df.columns:
        print("\n⚠️  Warning: Expected columns not found!")
        print("Looking for: 'sys_created_on' and 'u_service_offering_outage_start_time'")
    ...
    df['outage_start_time_clean'] = df['u_service_offering_outage_start_time'].apply(parse_datetime)
    
    # Calculate the difference
    print("Calculating differences...")
    df['difference_timedelta'] = df['sys_created_on_clean'] - df['outage_start_time_clean']
    ...
    print(f"Records with missing outage time: {df['outage_start_time_clean'].isna().sum()}")
    ...
    print("="*80)
    missing_outage = output_df[output_df['outage_start_time_clean'].isna()].head(5)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("  - u_service_offering_outage_start_time")
        print("\nThe script will:")
