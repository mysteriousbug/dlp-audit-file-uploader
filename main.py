...
def analytics_page(fs):
    """Analytics and audit page"""
    st.header("📊 File Analytics & Audit Trail")
    
    files = get_files_from_gridfs(fs)
    
    if not files:
        st.info("No files found for analysis.")
        return
    ...
    with col3:
        ...
        st.metric("Avg File Size", f"{avg_size / 1024:.2f} KB")
    
    with col4:
        classifications = df['classification'].value_counts()
        restricted_count = classifications.get('Restricted', 0) + classifications.get('Confidential', 0)
        st.metric("Sensitive Files", restricted_count)
    ...
    with col2:
        st.subheader("Files by Classification")
    ...
    recent_files = df.nlargest(10, 'upload_date')[['filename', 'size', 'upload_date', 'classification']]
    ...
    if st.button("📥 Export Audit Data"):
        ...
        st.download_button(
            label="Download CSV",
            data=csv,
            file_name=f"file_audit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv"
        )
